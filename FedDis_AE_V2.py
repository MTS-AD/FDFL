...
class DIS_LSTMAE(nn.Module):
    def __init__(self, n_features, hidden_size, n_layers, dropout):
        ...
        self.gl_att=SelfAttention(self.hidden_size,self.hidden_size,self.n_features)
        self.lo_att=SelfAttention(self.hidden_size,self.hidden_size,self.n_features)
    ...
    def forward(self, ts_batch,de_batch=None):
        ...
        output = self.gl_att(gl_dec[0])+self.lo_att(lo_dec[0])
...
if __name__ == '__main__':
    ...
    device=torch.device('cuda:'+argsx.device)
